...
async def latitude_data(tp, sec, latitude_data):
    try:
        message = prov.custom_data_request(sec, latitude_data)
        response = await tp.send_data('latitude-data', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None

async def longitude_data(tp, sec, longitude_data):
    try:
        message = prov.custom_data_request(sec, longitude_data)
        response = await tp.send_data('longitude-data', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None

async def appiid_data(tp, sec, appiid_data):
    try:
...
async def main():
    ...
    parser.add_argument('--latitude_data', dest='latitude_data', type=str,
                        help=desc_format(
                            'latitude_data'))

    parser.add_argument('--longitude_data', dest='longitude_data', type=str,
                        help=desc_format(
                            'longitude_data'))

    parser.add_argument('--appiid_data', dest='appiid_data', type=str, default='',
                        help=desc_format(
                            'appiid_data'))
    ...
    try:
        ...
        if args.latitude_data != '':
            print('\n==== Sending latitude_data to Target ====')
            if not await latitude_data(obj_transport, obj_security, args.latitude_data):
                raise RuntimeError('Error in custom data')
            print('==== latitude_data sent successfully ====')

        if args.longitude_data != '':
            print('\n==== Sending longitude data to Target ====')
            if not await longitude_data(obj_transport, obj_security, args.longitude_data):
                raise RuntimeError('Error in longitude data')
            print('==== longitude data sent successfully ====')

        if args.appiid_data != '':
            print('\n==== Sending appiid data to Target ====')
            if not await appiid_data(obj_transport, obj_security, args.appiid_data):
                raise RuntimeError('Error in appiid data')
            print('==== appiid data sent successfully ====')
