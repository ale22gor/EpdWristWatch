...
async def latlong_data(tp, sec, latlong_data):
    try:
        message = prov.custom_data_request(sec, latlong_data)
        response = await tp.send_data('latlong-data', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None

async def timezone_data(tp, sec, timezone_data):
    try:
        message = prov.custom_data_request(sec, timezone_data)
        response = await tp.send_data('timezone-data', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None
...
async def main():
    ...
    parser.add_argument('--latlong_data', dest='latlong_data', type=str, default='',
                        help=desc_format(
                            'This is an optional parameter, only intended for use with '
                            '"examples/provisioning/wifi_prov_mgr"'))

    parser.add_argument('--timezone_data', dest='timezone_data', type=str, default='',
                        help=desc_format(
                            'This is an optional parameter, only intended for use with '
                            '"examples/provisioning/wifi_prov_mgr"'))
    ...
    try:
        ...
        if args.latlong_data != '':
            print('\n==== Sending lattitude & longtitude data to Target ====')
            if not await latlong_data(obj_transport, obj_security, args.latlong_data):
                raise RuntimeError('Error in lattitude & longtitude data')
            print('==== Lattitude & longtitude data sent successfully ====')

        if args.timezone_data != '':
            print('\n==== Sending timezone data to Target ====')
